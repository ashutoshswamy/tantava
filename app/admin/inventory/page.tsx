  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 text-[#2b0e0a]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-[26px] font-bold text-[#2b0e0a] tracking-tight">Inventory</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
                ...
                <div key={product.id} className="bg-white border border-[#efdcb0] rounded-2xl p-4">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-[#930500]/10 text-[#930500] border border-[#930500]/30 rounded-xl text-[12px] font-medium hover:bg-[#930500]/20 transition-colors flex-shrink-0"
                    >
                      Adjust
                    </button>
                  </div>
          ...
          {/* Desktop table */}
          <div className="hidden sm:block bg-white border border-[#efdcb0] rounded-2xl overflow-x-auto">
            <table className="w-full min-w-[720px]">
              <thead>
                <tr className="border-b border-[#efdcb0] bg-[#fbf0da]/40">
                  ...
                  {SIZES.map((s) => (
                    <th key={s} className="text-center px-3 py-3.5 text-[11px] font-semibold text-[#8c6f52] uppercase tracking-wider">{s}</th>
                  ))}
                  <th className="text-left px-4 py-3.5 text-[11px] font-semibold text-[#8c6f52] uppercase tracking-wider">Status</th>
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-[#fbf0da]/30 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        <button
                          onClick={() => { setAdjustModal(product); setAdjustment({ size: "M", change: "", reason: "" }); }}
                          className="px-3 py-1.5 bg-[#930500]/10 text-[#930500] border border-[#930500]/30 rounded-xl text-[12px] font-medium hover:bg-[#930500]/20 transition-colors"
                        >
                          Adjust
                        </button>
                      </td>
        ...
        <div className="bg-white border border-[#efdcb0] rounded-2xl p-5 sm:p-6">
          <h2 className="text-[#2b0e0a] font-semibold text-[15px] mb-4">Recent Activity</h2>
          <div className="space-y-3 max-h-[500px] overflow-y-auto">
            {logs.length === 0 ? (
              <div className="text-center py-10">
                <p className="text-[#8c6f52] text-[13px]">No activity yet</p>
              </div>
            ) : (
              logs.map((log) => {
                const product = products.find((p) => p.id === log.product_id);
                return (
                  <div key={log.id} className="flex items-start gap-3 py-2.5 border-b border-[#efdcb0] last:border-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[#5c3b2e] text-[12px] font-medium truncate">{product?.name || "Unknown"}</p>
                      {log.size && <p className="text-[#8c6f52] text-[11px]">Size: {log.size}</p>}
                      <p className={`font-bold text-[13px] ${log.change > 0 ? "text-green-600" : "text-red-500"}`}>
                        {log.change > 0 ? "+" : ""}{log.change}
                      </p>
                      {log.reason && <p className="text-[#8c6f52] text-[11px]">{log.reason}</p>}
                      <p className="text-[#dcc9a0] text-[10px]">{new Date(log.created_at).toLocaleDateString("en-IN")}</p>
                    </div>
                  </div>
                );
              })
      ...
      {adjustModal && (
        <div
          className="fixed inset-0 z-50 bg-[#2b0e0a]/40 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={(e) => e.target === e.currentTarget && setAdjustModal(null)}
        >
          <div className="bg-white border border-[#efdcb0] rounded-2xl p-5 sm:p-7 w-full max-w-md shadow-xl">
            <h2 className="text-[#2b0e0a] font-bold text-[18px] mb-1">Adjust Stock</h2>
            <p className="text-[#8c6f52] text-[13px] mb-5">{adjustModal.name}</p>
            ...
            <form onSubmit={handleAdjust} className="space-y-4">
              <div>
                <label className="text-[11px] font-semibold text-[#8c6f52] uppercase tracking-wider mb-2 block">Size</label>
                    ...
                    <button
                      ...
                      className={`h-10 border rounded-xl font-semibold text-[12px] transition-colors ${
                        adjustment.size === size
                          ? "border-[#930500] bg-[#930500]/10 text-[#930500]"
                          : "border-[#dcc9a0] text-[#8c6f52] hover:border-[#930500]/40"
                      }`}
                    >
                      {size}
                    </button>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#fbf0da] border border-[#dcc9a0]/40 rounded-xl px-4 py-3 text-[#2b0e0a] text-[13px] focus:border-[#930500]/60 focus:outline-none transition-colors"
                  placeholder="e.g. +5 or -2"
                />
              </div>
              <div>
                <label className="text-[11px] font-semibold text-[#8c6f52] uppercase tracking-wider mb-1.5 block">Reason</label>
                <input
                  value={adjustment.reason}
                  onChange={(e) => setAdjustment({ ...adjustment, reason: e.target.value })}
                  className="w-full bg-[#fbf0da] border border-[#dcc9a0]/40 rounded-xl px-4 py-3 text-[#2b0e0a] placeholder:text-[#dcc9a0] text-[13px] focus:border-[#930500]/60 focus:outline-none transition-colors"
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3.5 bg-[#930500] text-white font-medium text-[14px] rounded-xl hover:bg-[#8c0500] transition-colors disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="px-6 py-3.5 border border-[#dcc9a0] text-[#8c6f52] rounded-xl hover:border-[#930500]/40 hover:text-[#2b0e0a] transition-colors font-medium text-[14px]"
                >
                  Cancel
                </button>
              </div>
