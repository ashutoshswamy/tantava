...
| `store_settings` | Singleton row (`id boolean primary key default true`) holding site-wide `delivery_info` / `returns_info` copy, `checkout_mode` / `whatsapp_number` (see Checkout flow below), `hero_images[]` (homepage hero banner carousel — `hero_image` also still present, kept as a legacy single-image fallback), `sale_ticker_text` / `sale_ticker_enabled` / `sale_ticker_color` / `sale_ticker_text_color` / `sale_ticker_speed_seconds` (homepage scrolling banner, fully styleable), `testimonials_enabled` (toggles the homepage testimonials section), `theme_background` / `theme_primary` / `theme_secondary` (three seed hex colors the full storefront palette is derived from), `first_purchase_discount_percent` (auto-applied at checkout for a signed-in user's first paid order, see Checkout flow), and `promo_modal_enabled` / `promo_modal_title` / `promo_modal_message` / `promo_modal_image` / `promo_modal_button_text` / `promo_modal_button_link` (storefront popup, admin-configurable) |
| `categories` | Admin-managed category taxonomy: `name`, `slug`, `is_active`, `sort_order` |
| `orders` | Customer orders — `items` JSON snapshot, `subtotal`/`total`, `coupon_code` / `discount_amount` (see Checkout flow), `status` enum, `shipping_address` JSON, Razorpay IDs, `admin_notes` |
| `coupons` | Discount codes — `code` (unique), `discount_type` (`percent`/`flat`), `discount_value`, `expires_at`, `is_active`. Service-role only (RLS enabled, no public policy) — codes are checked through `/api/coupons/validate`, never queried directly by clients |
...
| `collections`, `collections/[id]` | GET public (`collections` GET uses `"use cache: remote"`, backed by Upstash Redis); writes admin |
| `settings` | GET public (delivery & returns copy, checkout mode, hero images, sale ticker, testimonials toggle, theme color, promo modal, first-purchase discount); PUT admin |
| `inventory` | admin |
...
| `coupons`, `coupons/[id]` | admin — CRUD on discount codes |
| `coupons/validate` | signed-in user, rate-limited (20/10min) — checks a code and returns the computed discount for a given subtotal |
| `coupons/eligibility` | signed-in user — returns whether they qualify for the store's first-purchase discount (`store_settings.first_purchase_discount_percent`) |
| `admin/analytics`, `admin/stats` | admin — `admin/stats` inlines its own Clerk role check, same rule as `requireAdmin` |
| `admin/users` | admin — paginates through Clerk's user list (`clerkClient().users.getUserList`), returns name/email/phone/role/timestamps; inlines its own Clerk role check like `admin/stats` |
...
- **`razorpay`** (default):
  ...
  3. **`POST /api/razorpay/verify`** — signed-in user, rate-limited. Recomputes the HMAC-SHA256 signature of `order_id|payment_id` using `RAZORPAY_KEY_SECRET` and compares it with `crypto.timingSafeEqual`. Re-fetches the order from Razorpay's API and cross-checks the paid amount against a server-side recalculation of the cart total from the current `products` table (client-submitted prices are never trusted). Discount is also re-derived server-side, never taken from the client: if `orderData.coupon_code` was supplied, it re-validates the coupon (`lib/coupons-server.ts` `findActiveCoupon`) and recomputes the discount (`lib/discount.ts` `calcDiscount`); otherwise it checks `store_settings.first_purchase_discount_percent` and applies it if the user has no prior paid order (`hasCompletedOrder`). The Razorpay-charged amount must equal `recomputedTotal - discountAmount` or verification fails. On success: inserts the `orders` row (with `coupon_code`/`discount_amount`) and decrements `size_inventory` per line item.
...
Client-rendered pages under `/admin`, gated by `proxy.ts`. Sections (in nav order): Dashboard, Analytics, Products (list/new/edit), Categories, Collections, Coupons (list/new/edit, plus the first-purchase discount percent), Inventory, Orders, Users (read-only Clerk user list), Homepage (hero image carousel, sale ticker, promo modal, testimonials toggle), Appearance (storefront brand color), Checkout Settings (payment mode toggle), Delivery & Returns (site-wide copy editor), Feedback, Inquiries.
...
**Coupons** (`admin/coupons`): CRUD for the `coupons` table — code, discount type (percent/flat), value, expiry, active toggle. Also edits `store_settings.first_purchase_discount_percent`, the percentage auto-applied at checkout to a signed-in user's first paid order (no coupon code needed).

**Users** (`admin/users`): read-only list of every Clerk user (name, email, phone, role, created/last-sign-in), paginated server-side through Clerk's Backend API (`app/api/admin/users`). No in-app role editing — promote/demote admins from the Clerk dashboard.

**Homepage** (`admin/homepage`): edits the singleton `store_settings` row — hero banner image carousel (`hero_images[]`, multiple images, uploaded via the same signed-URL flow as product images, reorderable), sale ticker text/color/text-color/scroll-speed + on/off toggle, the storefront promo modal (enabled toggle, title, message, image, button text/link), and a toggle for whether the homepage testimonials section is shown.
